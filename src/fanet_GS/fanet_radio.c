...
void read_radio_data(sRadioData *radiodata) {
    ...
    radiodata->rssi = readReg(REG_LR_RSSIVALUE) -
                      157;        // Current RSSI value (dBm), RSSI[dBm] = -157 + Rssi (using HF output port)

    radiodata->psnr = (int8_t) (readReg(REG_LR_PKTSNRVALUE)) *
                      .25;    // Estimation of SNR on last packet received.In two’s compliment format mutiplied by 4.

    radiodata->prssi = readReg(REG_LR_PKTRSSIVALUE) - 157;
    if (radiodata->psnr <
        0)                                        // If SNR < 0 dB, correct Packet RSSI, see section 5.5.5 for details
        radiodata->prssi += radiodata->psnr;

    radiodata->rx_headers = readReg(
            REG_LR_RXHEADERCNTVALUEMSB);    // Number of valid headers received since last transition into Rx mode
    radiodata->rx_headers <<= 8;
    radiodata->rx_headers += readReg(REG_LR_RXHEADERCNTVALUELSB);

    radiodata->rx_packets = readReg(
            REG_LR_RXPACKETCNTVALUEMSB);    // Number of valid headers received since last transition into Rx mode
    ...
    switch (_cr) {
        case 1:
            strcpy(radiodata->coding_rate, "4/5");
            break;
        case 2:
            strcpy(radiodata->coding_rate, "4/6");
            break;
        case 3:
            strcpy(radiodata->coding_rate, "4/7");
            break;
        case 4:
            strcpy(radiodata->coding_rate, "4/8");
            break;
        default:
            strcpy(radiodata->coding_rate, "-/-");
            break;
    }
}
...
boolean read_rx_data(sRawMessage *_rx_radio, sRadioData *_radiodata) {
    ...
    uint8_t irq_flags = readReg(RF95_REG_12_IRQ_FLAGS);
    uint8_t _rx = irq_flags & RF95_RX_DONE;
    if (_rx)
    {
        _radiodata->timestamp = time(NULL);
#ifdef RF95_LED_PIN
        pinMode(RF95_LED_PIN, OUTPUT);
        digitalWrite(RF95_LED_PIN, HIGH );
#endif
        _rx_done = TRUE;
        ...
        byte currentAddr = readReg(
                REG_LR_FIFORXCURRENTADDR);    // Reads start address (in data buffer) of last packet received
        byte receivedCount = readReg(
                REG_LR_RXNBBYTES);            // Reads number of payload bytes of latest packet received
        _rx_radio->m_length = receivedCount;

        writeReg(REG_LR_FIFOADDRPTR,
                 currentAddr);                // Sets SPI interface address pointer in FIFO data buffer.
 ...
 * 
 *****************************************************************/
void setup_sx1276_LoRa(boolean _show_msg) {
...
void init_fanet_radio_b(int power, boolean _show_msg) {
    init_rpi_spi();
    setup_sx1276_LoRa(_show_msg);
...
void init_fanet_radio(int _delay, boolean _show_msg, int _power) {
    ...
    init_fanet_radio_b(_power, _show_msg);

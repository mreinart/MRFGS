...
#define VTSEQ(ID) ("\x1b[" #ID "m")
	...
	static Text vtextprintf(const char *format, va_list vlist) {
		...
		CHECK_F(result >= 0, "Bad string format: '"
				LOGURU_FMT(s)
				"'", format);
		return Text(buff);
#endif
		...
		for (int arg_it = 1; arg_it < argc; ++arg_it) {
			...
			if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !std::isalpha(cmd[arg_len], std::locale(""))) {
				...
				if (value_str[0] == '\0') {
					...
					arg_it += 1;
					CHECK_LT_F(arg_it, argc, "Missing verbosiy level after "
							LOGURU_FMT(s)
							"", verbosity_flag);
...
#define getcwd _getcwd
		...
		if (!getcwd(s_current_dir, sizeof(s_current_dir))) {
			...
			LOG_F(WARNING, "Failed to get current working directory: "
					LOGURU_FMT(s)
					"", error_text.c_str());
		}
		...
		VLOG_F(g_internal_verbosity, "arguments: "
				LOGURU_FMT(s)
				"", s_arguments.c_str());
		if (strlen(s_current_dir) != 0) {
			VLOG_F(g_internal_verbosity, "Current dir: "
					LOGURU_FMT(s)
					"", s_current_dir);
		}
		VLOG_F(g_internal_verbosity, "stderr verbosity: "
				LOGURU_FMT(d)
				"", g_stderr_verbosity);
	...
	bool add_file(const char *path_in, FileMode mode, Verbosity verbosity) {
		...
		if (!create_directories(path)) {
			LOG_F(ERROR, "Failed to create directories to '"
					LOGURU_FMT(s)
					"'", path);
		}
		...
		VLOG_F(g_internal_verbosity, "Logging to '"
				LOGURU_FMT(s)
				"', mode: '"
				LOGURU_FMT(s)
				"', verbosity: "
	...
	bool remove_callback(const char *id) {
		...
		if (it != s_callbacks.end()) {
			...
			LOG_F(ERROR, "Failed to locate callback with id '"
					LOGURU_FMT(s)
					"'", id);
			return false;
		}
